module RequestLogAnalyzer::FileFormat
  ...
  class RailsPa < Base
    ...
    line_definition :started do |line|
      ...
      line.regexp = /(?:\[([0-9a-f]{10})\] \[(.*)\] \[(.{10})\]) Started ([A-Z]+) "([^"]+)" for (#{ip_address}) at (#{timestamp('%a %b %d %H:%M:%S %z %Y')}|#{timestamp('%Y-%m-%d %H:%M:%S %z')})/
      line.capture(:UUID)
      line.capture(:employer)
      line.capture(:session)
      line.capture(:method)
      line.capture(:path)
      line.capture(:ip)
